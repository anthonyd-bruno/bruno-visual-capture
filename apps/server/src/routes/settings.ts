...
export function registerSettingsRoutes(app: FastifyInstance, ctx: ServerContext): void {
  const view = () => ({ settings: ctx.settings.get(), secrets: { openai: { keyPresent: ctx.aiKeyPresence().openai }, anthropic: { keyPresent: ctx.aiKeyPresence().anthropic } }, paths: { root: ctx.paths.root, artifactRoot: ctx.settings.artifactRoot() } });
