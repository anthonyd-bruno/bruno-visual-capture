...
5. Fixtures: use a bundled fixture when its collection already has what the prompt needs. Use an inline collection when the prompt needs specific requests, headers, bodies, auth or environments — write it as JSON in inlineCollectionJson (Bruno Capture writes the files; requests may use {{variables}} from the environments; a request may also carry "variables" [{name, value}], "scripts" {beforeRequest, afterResponse, tests} (Bruno's bru/req/res/test/expect API) and "assertions" [{expression, operator, value}] so the Vars/Script/Assert/Tests tabs and the response Tests tab have content; public demo APIs such as https://jsonplaceholder.typicode.com (and https://httpbin.org for auth echoes) make sends succeed; a single environment is pre-selected automatically). Use fixtureKind none only for prompts about creating the very first collection.
