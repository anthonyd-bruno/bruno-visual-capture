...
- Secret auth fields (bearer token, basic password, API-key value) are MASKED as **** by default. To make the value readable in a capture use request.setAuth {…, reveal: "true"} or request.revealSecret (clicks the eye button, testId secret-reveal-toggle) after the value is entered.
